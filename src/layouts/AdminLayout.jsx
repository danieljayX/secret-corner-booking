...
export default function AdminLayout() {
  ...
  const sidebarBg = isDarkMode ? 'bg-slate-900' : 'bg-[#1E1B4B]'; 
  ...
  return (
    <div className={`min-h-[100dvh] ${bg} ${textColor} font-['Plus_Jakarta_Sans',_sans-serif] flex transition-colors duration-500`}>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        {/* Mobile Header */}
        <div className={`lg:hidden sticky top-0 z-40 ${headerBg} backdrop-blur-md border-b ${isDarkMode ? 'border-slate-800' : 'border-slate-100'} px-6 py-5 flex items-center justify-between shadow-sm transition-all`}>
          <div className="flex items-center gap-5">
             <button className={`${isDarkMode ? 'text-white' : 'text-slate-800'} flex flex-col gap-1.5`}>
               <div className={`w-6 h-[2.5px] ${isDarkMode ? 'bg-white' : 'bg-slate-800'} rounded-full`}></div>
               <div className={`w-4 h-[2.5px] ${isDarkMode ? 'bg-white' : 'bg-slate-800'} rounded-full`}></div>
               <div className={`w-6 h-[2.5px] ${isDarkMode ? 'bg-white' : 'bg-slate-800'} rounded-full`}></div>
             </button>
             <h1 className={`text-[20px] font-black ${isDarkMode ? 'text-white' : 'text-slate-800'} tracking-tight`}>Dashboard</h1>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              ...
              <button className={`p-1 rounded-xl ${isDarkMode ? 'text-white bg-slate-800' : 'text-slate-800 bg-slate-50'}`}>
        ...
        {/* Mobile Bottom Nav */}
        <div className={`lg:hidden sticky bottom-0 z-40 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'} border-t shadow-[0_-10px_40px_rgba(0,0,0,0.04)] px-4 pb-2 transition-all`}>
