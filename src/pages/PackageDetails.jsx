  ...
  return (
    <div className={`min-h-screen ${bg} pb-24 font-['Inter'] transition-colors duration-300 ${textColor}`}>
      ...
      <div className="px-6 py-4 space-y-8">
        {/* Package Title Card */}
        <div className={`relative overflow-hidden rounded-[2.5rem] p-12 text-center transition-all duration-300 border ${isDarkMode ? 'bg-white/5 border-white/10 shadow-2xl' : 'bg-white border-gray-100 shadow-lg'}`}>
          {/* Background Glow */}
          ...
          <div className="relative z-10 space-y-4">
            <div className={`inline-flex items-center gap-2 px-4 py-1 rounded-full border mb-2 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-gray-50 border-gray-100'}`}>
            ...
            <h1 className={`text-5xl font-black ${colorClass} uppercase italic tracking-widest leading-none ${isDarkMode ? 'drop-shadow-[0_0_15px_rgba(0,0,0,0.5)]' : ''}`}>
              {pkg.name}
            </h1>
            <p className={`text-3xl font-black tracking-tight ${textColor}`}>₱{pkg.price.toLocaleString()}</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className={`rounded-[2.5rem] p-8 space-y-6 border transition-all duration-300 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white border-gray-100 shadow-sm'}`}>
            {pkg.features.map((feature, idx) => (
              <div key={idx} className="flex items-center gap-5">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-all ${isDarkMode ? 'bg-white/5 border-white/5' : 'bg-gray-50 border-gray-100'} ${colorClass}`}>
                  <CheckCircle2 size={20} />
                </div>
                <span className={`text-sm font-black uppercase tracking-widest leading-snug ${isDarkMode ? 'text-white' : 'text-gray-700'}`}>{feature}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Exclusive Badge */}
        <div className={`rounded-3xl p-6 flex items-start gap-5 border transition-all ${isDarkMode ? 'bg-violet-600/10 border-violet-500/20' : 'bg-indigo-50 border-indigo-100'}`}>
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${isDarkMode ? 'bg-violet-500/20' : 'bg-white shadow-sm border border-indigo-50'}`}>
            <ShieldCheck className={isDarkMode ? 'text-violet-400' : 'text-indigo-600'} size={24} />
          </div>
          <div>
        ...
        <button
          ...
          className={`w-full ${isDarkMode ? 'bg-gradient-to-r from-violet-600 to-indigo-600 shadow-[0_0_30px_rgba(139,92,246,0.4)]' : 'bg-indigo-600 shadow-[0_8px_30px_rgba(79,70,229,0.2)]'} text-white font-black uppercase tracking-[0.2em] py-6 rounded-[2rem] hover:opacity-90 transition-all flex items-center justify-center gap-4 active:scale-95 mt-4 group`}
        >
